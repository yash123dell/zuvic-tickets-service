...
// - UI_SESSION_SECRET (signing key for admin cookie; falls back to ADMIN_UI_KEY)
...
app.post(`${PROXY_MOUNT}/attach-ticket`, async (req,res)=>{
  try{
    ...
    const {
      order_id, ticket_id, status="pending", issue="", message="",
      phone="", email="", name="", order_name="", created_at
    } = req.body || {};
    ...
    const d2 = await adminGraphQL(`
      ...
      { ownerId: orderGid, value: JSON.stringify(map), ticketId: ticket_id, status }
    );
...
app.get(`${PROXY_MOUNT}/find-ticket`, async (req,res)=>{
  try{
    ...
    if (raw) {
      try { const map = JSON.parse(raw); ticket = map?.[ticket_id]; status = ticket?.status || null; }
      catch {}
    }
...
async function collectTickets({ since, status, limit=200 }){
  ...
  while(out.length < max){
    ...
    for(const { cursor, node } of edges){
      ...
      let map={}; const raw=node.mfJSON?.value;
      ...
      if (Object.keys(map).length){
        ...
        const rec = { ...base, ticket_id: node.mfId.value, status: node.mfSt?.value || "pending", issue:"", message:"", phone:"", email:"", name:"", created_at: base.order_created_at, updated_at: base.order_updated_at };
...
// ---------- Admin panel auth (Basic + session cookie) ----------
const UI_USER = process.env.UI_USER || "admin";
const UI_PASS = process.env.UI_PASS || "change-me";
const UI_SESSION_SECRET = process.env.UI_SESSION_SECRET || ADMIN_UI_KEY || "change-me-too";
...
function requireUIPassword(req,res,next){
  ...
  if (user===UI_USER && pass===UI_PASS) {
    // issue/refresh session cookie for XHRs
    const token = makeToken(12);
    res.cookie("ui_session", token, { httpOnly:true, sameSite:"Strict", secure:true, path:"/admin" });
    return next();
  }
  res.set("WWW-Authenticate", 'Basic realm="Tickets Admin"');
  return res.status(401).send("Auth required");
...
// ---------- Inline Admin Panel (no external file needed) ----------
...
<style>
  :root{ --bg:#f6f7fb; --fg:#0f172a; --muted:#64748b; --card:#fff; --border:#e5e7eb; --primary:#1d4ed8; --pill:#eef2ff; --pillfg:#3730a3; }
...
<div class="wrap">
  ...
  <div class="card">
    <div class="filters">
      ...
      <label> Updated since <input id="since" type="date"/></label>
      <label> Limit <input id="lim" type="number" value="200" min="1" max="1000"/></label>
      <label> Search (ticket/order/name/email) <input id="q" placeholder="Type to filter…"/></label>
    ...
    <div style="margin-top:10px;display:flex;gap:8px;align-items:center">
      ...
      <select id="bulkStatus" style="width:180px">
        <option value="pending">pending</option><option value="in_progress">in_progress</option><option value="resolved">resolved</option><option value="closed">closed</option>
      </select>
    ...
    <div class="table-wrap">
      <table id="tbl">
        <thead>
          <tr>
            <th style="width:36px"></th><th>TICKET</th><th>STATUS</th><th>ORDER</th><th>CREATED</th><th>UPDATED</th><th>NAME</th><th>EMAIL</th><th>PHONE</th><th>ISSUE</th><th>MESSAGE</th><th style="width:200px">ACTIONS</th>
          </tr>
        </thead>
...
function esc(v){ const m={'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}; return String(v??"").replace(/[&<>"']/g, s=>m[s]); }
const fmt  = (d)=> d ? new Date(d).toLocaleString() : "—";
const show = (msg)=>{ const t=$("#toast"); t.textContent=msg; t.classList.add("show"); setTimeout(()=>t.classList.remove("show"), 1200); };

function pill(s){ return '<span class="pill">'+esc(String(s||"").replace("_"," "))+'</span>'; }
...
function filterRows(q,list){ if(!q) return list; const k=q.toLowerCase(); return list.filter(t=>[t.ticket_id,t.order_name,t.name,t.email].filter(Boolean).some(x=>String(x).toLowerCase().includes(k))); }
...
async function load(){
  ...
  try{
    ...
    $("#tbl tbody").innerHTML = (filterRows($("#q").value||"", j.tickets).map(row).join("")) || '<tr><td colspan="12" class="muted">No tickets</td></tr>';

    $$("#tbl .save").forEach(b=>b.onclick=async(e)=>{ const tr=e.target.closest("tr"); const body={ order_id:tr.dataset.oid, ticket_id:tr.dataset.tid, status: tr.querySelector(".set").value };
      ...
      const j2=await r2.json(); if(!j2.ok) alert("Update failed: "+j2.error); else { show("Updated"); load(); } });
    $$("#tbl .copy").forEach(b=>b.onclick=(e)=>{ const tr=e.target.closest("tr"); const data={ ticket:tr.dataset.tid, order:tr.dataset.oid, status:tr.querySelector(".set").value, name:tr.children[6].innerText, email:tr.children[7].innerText, phone:tr.children[8].innerText, issue:tr.children[9].innerText, message:tr.children[10].innerText }; navigator.clipboard.writeText(JSON.stringify(data,null,2)); show("Copied"); });
...
$("#applyBulk").onclick=async()=>{ const status=$("#bulkStatus").value; const rows=Array.from($$("#tbl .sel")).filter(cb=>cb.checked).map(cb=>cb.closest("tr"));
  for(const tr of rows){ const body={ order_id:tr.dataset.oid, ticket_id:tr.dataset.tid, status }; await fetch("/admin/ui/update",{ method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify(body), credentials:"include" }); }
  show("Bulk updated"); load();
};
...
// JSON used by the inline panel (accept cookie OR Basic)
