...
// Basic on first load, then signed cookie; also accepts Basic on XHR.
// ======================================================================
...
function requireUIPassword(req, res, next) {
  res.set("Cache-Control", "no-store, must-revalidate"); res.set("Pragma","no-cache");
  const hdr = req.headers.authorization || "";
  if (!hdr.startsWith("Basic ")) {
    res.set("WWW-Authenticate", 'Basic realm="Tickets Admin"'); return res.status(401).send("Auth required");
  }
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  select,input,button{width:100%;height:36px;border:1px solid var(--border);border-radius:8px;padding:0 10px;background:#fff;color:#fg}
...
// ✅ FIXED: proper quote escaping (no runtime error)
